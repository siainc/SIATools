...
+ (NSString *)stringWithLength:(NSInteger)length left:(NSString *)left center:(NSString *)center right:(NSString *)right
{
    ...
    if (center) {
        string = [string stringByReplacingCharactersInHalfwidthRange:NSMakeRange((string.halfwidthLength - center.halfwidthLength) / 2,
                                                                                 center.halfwidthLength)
                                                          withString:center];
    }
    if (right) {
        string = [string stringByReplacingCharactersInHalfwidthRange:NSMakeRange(string.halfwidthLength - right.halfwidthLength,
                                                                                 right.halfwidthLength) withString:right];
    }
    return string;
}
