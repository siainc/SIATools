...
+ (UIColor *)colorWithHexString:(NSString *)hexString
{
    ...
    if (validateError) {
        SIAToolsDLog(@"文字列のバリデードチェックregexエラー:%@", validateError);
        return nil;
    }
    ...
    if (validateResult.numberOfRanges == 0) {
        SIAToolsDLog(@"文字列が有効ではない:%@", hexString);
        return nil;
    }
    ...
    if (value.length == 6 || value.length == 8) {
        ...
        rgbaMax = 255.0;
        regex   =
        [NSRegularExpression regularExpressionWithPattern:@"([0-9A-Fa-f]{2})([0-9A-Fa-f]{2})([0-9A-Fa-f]{2})([0-9A-Fa-f]{2})?" options:0 error:&
         error];
    }
    else if (value.length == 3 || value.length == 4) {
        // rgb (max 15)
        rgbaMax = 15.0;
        regex   = [NSRegularExpression regularExpressionWithPattern:@"([0-9A-Fa-f])([0-9A-Fa-f])([0-9A-Fa-f])([0-9A-Fa-f])?" options:0 error:&error];
    }
    if (error) {
        SIAToolsDLog(@"regex error:%@", error);
        return nil;
    }
