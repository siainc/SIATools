...
+ (NSDateFormatter *)cachedObjectWithFormat:(NSString *)format locale:(NSLocale *)locale timeZone:(NSTimeZone *)timeZone
{
    ...
    NSDateFormatter *formatter = [_siatoolsDateFormatterPool objectOfObjectPassingTest:^BOOL(id obj, NSUInteger idx, BOOL *stop) {
        ...
        return [self dateFormatterWithFormat:format locale:locale timeZone:timeZone];
    }];
    
    [_siatoolsDateFormatterPool removeObject:formatter];
    [_siatoolsDateFormatterPool pushObject:formatter];
    if (_siatoolsDateFormatterPool.count > 5) {
        [_siatoolsDateFormatterPool shiftObject];
    }
    return formatter;
}
